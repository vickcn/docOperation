...
class DocRebuilder:
    ...
    def rebuild_docx(self, data: Dict[str, Any], output_path: str) -> None:
        ...
        try:
            ...
            # 應用節屬性（包括分欄設置）
            self._apply_section_properties(doc, data.get("sections", []))
    ...
    def _apply_section_properties(self, doc: Document, sections: list) -> None:
        """應用文檔節的屬性，包括分欄設置"""
        if not sections:
            return
            
        # 確保文檔至少有一個節
        while len(doc.sections) < len(sections):
            doc.add_section()
            
        # 應用每個節的屬性
        for i, section_props in enumerate(sections):
            section = doc.sections[i]
            
            # 設置頁面大小和邊距
            section.page_height = Pt(section_props["page_height"])
            section.page_width = Pt(section_props["page_width"])
            section.left_margin = Pt(section_props["left_margin"])
            section.right_margin = Pt(section_props["right_margin"])
            section.top_margin = Pt(section_props["top_margin"])
            section.bottom_margin = Pt(section_props["bottom_margin"])
            section.header_distance = Pt(section_props["header_distance"])
            section.footer_distance = Pt(section_props["footer_distance"])
            ...
            cols_element = parse_xml(f'<w:cols {nsdecls("w")} w:num="{section_props["columns"]["count"]}" w:space="{section_props["columns"]["space"]}"/>')
